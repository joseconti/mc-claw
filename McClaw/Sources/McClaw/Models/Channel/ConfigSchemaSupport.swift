...
/// Path segment for navigating a config schema tree.
enum ConfigPathSegment: Hashable, Sendable {
    case key(String)
    case index(Int)
}

/// A path through a config schema (e.g. ["channels", "whatsapp", "dmPolicy"]).
typealias ConfigPath = [ConfigPathSegment]

/// UI hint metadata attached to config schema fields.
struct ConfigUiHint: Sendable {
    let label: String?
    let help: String?
    let order: Double?
    let advanced: Bool?
...
/// A JSON Schema node for dynamic config form generation.
struct ConfigSchemaNode: @unchecked Sendable {
    let raw: [String: Any]
    ...
    var description: String? { raw["description"] as? String }

    var enumValues: [Any]? { raw["enum"] as? [Any] }
    var constValue: Any? { raw["const"] }
    var explicitDefault: Any? { raw["default"] }
    ...
    var typeList: [String] {
        if let type = raw["type"] as? String { return [type] }
        if let types = raw["type"] as? [String] { return types }
        return []
    }

    var schemaType: String? {
        let filtered = typeList.filter { $0 != "null" }
    ...
    var items: ConfigSchemaNode? {
        if let items = raw["items"] as? [Any], let first = items.first {
    ...
    var additionalProperties: ConfigSchemaNode? {
        if let additional = raw["additionalProperties"] as? [String: Any] {
    ...
    var allowsAdditionalProperties: Bool {
        if let allow = raw["additionalProperties"] as? Bool { return allow }
        return additionalProperties != nil
    }

    var defaultValue: Any {
        if let value = raw["default"] { return value }
        switch schemaType {
    ...
    /// Navigate to a nested schema node by path.
...
/// Decode raw UI hints dictionary.
...
/// Resolve a UI hint for a specific path, including wildcard matching.
...
/// Check if a config path looks like a sensitive field.
...
/// Convert a config path to a dot-separated key string.
